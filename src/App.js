...
Generated by ECP Assistant`;
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    // Solo auto-extraer si NO estamos editando y es una URL válida
                    if (!editingProduct && e.target.value && (e.target.value.startsWith('http://') || e.target.value.startsWith('https://'))) {
  ...
  {/* Modal Product Detail */}
      ...
      {/* Modal Pack Detail - ENHANCED VERSION */}
                ...
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => {
                      ...
                      addPackToEstimate(selectedPack);
                      setShowPackDetail(false);
                      setSelectedPack(null);
                    }}
                    className="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors text-sm"
                  >
                    Add All to New Estimate
                  </button>
                </div>
                    ...
                    {selectedPack.products.map((product, index) => (
                      <div key={index} className="flex items-center justify-between p-4 bg-slate-50 rounded-lg">
                        <div className="flex-1">
                          ...
                          <p className="text-slate-500 text-sm">Price: ${Number(product.price).toFixed(2)} each</p>
                        </div>
                        
                        <div className="flex items-center gap-4">
                          <div className="flex items-center gap-2">
                            <span className="font-medium text-slate-800 min-w-[2rem] text-center">1</span>
                          </div>
                          ...
                          <button
                            onClick={() => {
                              const realProduct = products.find(p => p.name === product.name) || product;
                              addProductToEstimate(realProduct);
                              showNotification(`Added ${product.name} to estimate`, 'success');
                            }}
                            className="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors"
                            title="Add this product to estimate"
                          >
                            <Plus className="w-4 h-4" />
                          </button>
                        </div>
